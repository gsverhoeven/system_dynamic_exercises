...
We start with a delays set to zero. If the response delay is zero, 

```{r}
source("models/business_inventory.R")

# Figure 30
auxs <- c(aPerception_delay = 0, # days or minimum of 1?
          aResponse_delay = 1,
          aDelivery_delay = 0)
...
ggplot(o, aes(x = time, y = sInventory_of_cars_on_the_lot)) +
  ...
  labs(x = "Days", y = "Cars on the lot")
```
The manager then decides to reduces the response delay, making up perceived shortfalls in 2 instead of3 days.
...
She should react LESS rapidly, for example by increasing the response delay from 3 to 6 
Now the oscillations are damped out.
